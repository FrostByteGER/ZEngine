	...
	public abstract class PhysicsComponent : RenderComponent, ICollidable
	{
		...
		public TVector2f Velocity
		{
			...
			set
			{
				...
				CollisionBody.LinearVelocity = PhysicsWorld.ToPhysicsUnits(linVel);
			}
		}
		...
		{
			get => PhysicsWorld.ToGameUnits(CollisionBody.AngularVelocity);
			set
			{
				var angVel = value;
				// Clamp the value to +-MaxAngularVelocity before assigning it.
				angVel = angVel.Clamp(-MaxAngularVelocity, MaxAngularVelocity);
				CollisionBody.AngularVelocity = PhysicsWorld.ToPhysicsUnits(angVel);
			}
		}
		...
		{
			get => PhysicsWorld.ToGameUnits(CollisionBody.LinearDamping);
			set => CollisionBody.LinearDamping = PhysicsWorld.ToPhysicsUnits(value);
		}
		...
		{
			get => PhysicsWorld.ToGameUnits(CollisionBody.AngularDamping);
			set => CollisionBody.AngularDamping = PhysicsWorld.ToPhysicsUnits(value);
		}
		...
		public override TVector2f LocalPosition
		{
			...
			set
			{
				...
				CollisionBody.Position = PhysicsWorld.ToPhysicsUnits(value);
			}
		}
		...
		public override float LocalRotation
		{
			...
			set
			{
				...
				CollisionBody.Rotation = PhysicsWorld.ToPhysicsUnits(EngineMath.DegreesToRadians(value)); //TODO: Verify
			}
		}
